...
import { fetchDomainFlow } from "../api/graph";
...
export function SankeyDomainFlow(): JSX.Element {
  ...
  const [counts, setCounts] = useState<{ domains: number; flows: number }>({
    domains: 0,
    flows: 0,
  });
  ...
  useEffect(() => {
    ...
    (async (): Promise<void> => {
      try {
        const res = await fetchDomainFlow(ac.signal, 50000);
        ...
        setCounts({ domains: res.nodes.length, flows: res.links.length });

        const idx = new Map(res.nodes.map((n, i) => [n.id, i] as const));
        const nodes: SNode[] = res.nodes.map((n) => ({ name: n.domain }));
        const links: SLink[] = [];
    ...
    <div className="vz-view vz-view--sankey">
      ...
      {status === "loading" && (
        <div className="vz-view-hint" role="status">
          loading graph.db domain-to-domain edges...
        </div>
      )}
      {status === "empty" && (
        <div className="vz-view-error" role="status">
          no cross-domain edges in shard yet -- run <code>mneme build .</code>
        </div>
      )}
      {status === "error" && error && (
      ...
      {status === "ready" && (
        <p className="vz-view-hint">
          {counts.domains.toLocaleString()} domains - {counts.flows.toLocaleString()} flows
        </p>
      )}
    </div>
  );
