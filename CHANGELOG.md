...
The audit comparing mneme to CRG and graphify (2026-05-05) identified one root cause behind both gaps — recall (mneme 2/10 vs CRG 6/10) and token reduction (mneme 1.34× vs CRG's claimed 6.8×). Mneme had no symbol resolver: function names matched README chunks more strongly than the function itself. This release ships the **resolver libraries** that close that gap; the **extractor wiring** that makes them effective on the graph-query side (`find_references`, `blast_radius`, `call_graph`) is **deferred to v0.4.1** (see "Deferred" section below). The embedding-side win lands in v0.4.0 because the embed pipeline does prepend resolver canonical prefixes via Item #117.

> **Honest framing (2026-05-05 deep audit pass):** the 2/10 → 6/10 number is the audit's projection of what the resolver chain *will* deliver once it's wired through `extractor.rs`. v0.4.0 ships the resolver code as a library and uses it for embedding-side anchoring; the graph-side wiring lands in v0.4.1.

- **Symbol resolver — Rust** (Item #114). New `parsers/src/resolver.rs::RustResolver` rewrites syntactic paths (`super::`, `self::`, `crate::`, `use` aliases) into one canonical string per logical symbol. Walks file path → `crate::module::path::to::file` prefix and rewrites references against a `UseMap` derived from the file's `use` statements. **Library only in v0.4.0** — wired into `extractor.rs` in v0.4.1.
- **Symbol resolver — TypeScript / JavaScript** (Item #115). `TypeScriptResolver` handles tsconfig `paths` aliases (wildcard + exact match), relative imports (`./x`, `../y`), and bare module specifiers. Per-language file-prefix function (filename kept since TS files aren't directory-as-module). **Library only in v0.4.0** — same v0.4.1 wiring path as Rust.
- **Symbol resolver — Python** (Item #116). `PythonResolver` handles N-leading-dot relative imports (walking one parent up per dot), absolute imports, and aliased imports via `PythonImportMap`. Native `.` separator for round-trip compatibility with jedi/mypy/sphinx. **Library only in v0.4.0** — same v0.4.1 wiring path.
- **Symbol-anchored BGE embeddings** (Item #117). The embedder now prepends the resolver's canonical prefix (`crate::manager::WorkerPool` / `vision/src/views/Foo.tsx::Bar` / `pkg.sub.mod.spawn`) before signature/summary text, so `recall_concept "spawn"` matches the actual function instead of the README. Falls back to file-path-anchored text for languages without a resolver yet. **This IS wired in v0.4.0** — it's the embed pass's path, distinct from the extractor wiring deferred above.
...
- **Auto-update apply mode + post-swap health check + rollback** (Item #84). `mneme self-update` now verifies the freshly-installed binary by running it with `--version` (5-second timeout, stdin/stdout/stderr piped to null). On non-zero exit or timeout, every `.old` backup is restored over the corresponding new binary, leaving the user exactly where they started. First-install rollback deletes the new file. Test seam (`replace_binaries_atomically_with_check`) so unit tests drive both paths without spawning real processes.
