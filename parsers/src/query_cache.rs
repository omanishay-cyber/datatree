...
fn pattern_for(lang: Language, kind: QueryKind) -> &'static str {
    match (lang, kind) {
        ...
        (Language::TypeScript | Language::Tsx, QueryKind::Imports) => {
            // P3.6: broaden — match plain import_statement (covers
            // side-effect imports + lets K7 binding walker count
            // import_specifiers), re-exports with `from`, dynamic
            // import("./x"), and CJS require("./x"). The narrow
            // `source: (string) @source` predicate that lived here
            // before missed all four extras and undercounted edges
            // ~10x on real TS+React codebases.
            r#"
            (import_statement) @import
            (export_statement source: (string) @source) @import
            (call_expression function: (import)) @import
            (call_expression function: (identifier) @_id (#eq? @_id "require")) @import
            "#
        }
        ...
        (Language::JavaScript | Language::Jsx, QueryKind::Imports) => {
            // P3.6: see TS branch above — same broadening rationale.
            r#"
            (import_statement) @import
            (export_statement source: (string) @source) @import
            (call_expression function: (import)) @import
        ...
        (Language::Rust, QueryKind::Imports) => {
            // P3.6: include extern_crate_declaration alongside use.
            // mod_item intentionally NOT included — it declares a
            // child module (structural CONTAINS), not a cross-file
            // import edge.
            r#"
