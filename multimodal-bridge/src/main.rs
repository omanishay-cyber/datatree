...
#[command(
    ...
    long_about = "Pure-Rust multimodal extraction for mneme.\n\
                  \n\
                  Default features: PDF and Markdown only — no system libraries required.\n\
                  \n\
                  Optional extractors (opt-in via cargo features; not enabled in published binaries):\n\
                  \n\
                  * `tesseract` — OCR over images. Requires libtesseract + leptonica system\n\
                    packages. Without this feature the ImageExtractor records dimensions only\n\
                    (no OCR text). Enable with `cargo build --features tesseract`.\n\
                  \n\
                  * `whisper` — audio transcription via whisper.cpp. Requires a C++ toolchain\n\
                    plus a Whisper GGML model on disk; the path is set by the caller. Enable\n\
                    with `cargo build --features whisper`.\n\
                  \n\
                  * `ffmpeg` — video frame sampling + audio decoding via libavformat /\n\
                    libavcodec / libavutil. Enable with `cargo build --features ffmpeg`.\n\
                  \n\
                  Use `--features all-extractors` to enable every optional extractor at once.\n\
                  Tesseract/Whisper/ffmpeg bundling for v0.3.x ships intentionally turned off:\n\
                  the system-library footprint is too large for the default install. v0.4 may\n\
                  ship a 'fat' build with Tesseract bundled; until then, `mneme-multimodal`\n\
                  will silently fall back to dimensions-only when an image is fed in without\n\
                  the OCR feature compiled in."
)]
struct Cli {
    #[command(subcommand)]
    command: Cmd,
