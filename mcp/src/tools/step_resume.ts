...
export const tool: ToolDescriptor<StepResumeInputT, StepResumeOutputExtendedT> = {
  ...
  async handler(input, ctx) {
    ...
    //
    // F2 fix (NEW-052): bound the IPC fallback to a single 2-second budget so
    // the tool always returns inside the test driver's 6 s ceiling. The
    // composer issues 3 sequential IPC reads which, with a 2 s per-call
    // timeout, can otherwise stack to ~6 s and trip the driver's deadline.
    const FALLBACK_DEADLINE_MS = 2_000;
    try {
      const ipcWithDeadline = await Promise.race<{
        bundle: string;
        current_step_id: string | null;
        total_steps: number;
      } | null>([
        buildResume({ cwd: ctx.cwd, sessionId }),
        new Promise((resolve) =>
      ...
      if (ipcWithDeadline) {
        return {
          bundle: ipcWithDeadline.bundle,
          current_step_id: ipcWithDeadline.current_step_id,
          total_steps: ipcWithDeadline.total_steps,
