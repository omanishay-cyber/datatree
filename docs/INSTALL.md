...
7. Register the MCP server with Claude Code by writing ONE entry to
   `~/.claude.json`. **No settings.json write. No hooks registered.**
...
```
~/.mneme/
|- bin/                     pre-built binaries (mneme, mneme-daemon, 9 workers)
|- mcp/                     Bun TS MCP server (47 tools)
|- plugin/
|  |- plugin.json
...
Mneme does **not** touch `~/.claude/settings.json`. The v0.3.0 install
incident (see `mneme-install-report/`) was caused by hook registration
in that file; v0.3.1+ architecturally prevents it.
