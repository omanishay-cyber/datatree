//! Datatree CLI — library surface.
//!
//! The `datatree` binary in `main.rs` is intentionally thin: it parses
...
//! 3. expose a stable surface for future plugins that want to embed datatree.
//!
//! ## Module map
//!
...
//! - [`markers`]    — idempotent injection: `<!-- datatree-start v1.0 --> … -->`
...
/// Marker version embedded inside `<!-- datatree-start v{VERSION} -->`.
...
pub const DEFAULT_IPC_SOCKET_NAME: &str = "datatree-supervisor.sock";
...
pub fn runtime_dir() -> std::path::PathBuf {
    ...
    if let Some(home) = dirs::home_dir() {
        return home.join(".datatree").join("run");
    }
    std::env::temp_dir().join("datatree-run")
}
...
pub fn state_dir() -> std::path::PathBuf {
    ...
    if let Some(home) = dirs::home_dir() {
        return home.join(".datatree");
    }
    std::env::temp_dir().join("datatree-state")
}
