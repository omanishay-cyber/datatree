...
//! ## Why markers?
//!
//! From design §25.5 (carried over from graphify): re-running `mneme
//! install` MUST NOT duplicate the mneme section. We solve this with two
//! sentinels:
//!
//! ```text
//! <!-- mneme-start v1.0 sha256=<hex> -->
//! ...generated body...
//! <!-- mneme-end -->
//! ```
//!
...
pub const MARKER_START_PREFIX: &str = "<!-- mneme-start ";
/// Closing sentinel — full literal because there's no version suffix here.
pub const MARKER_END: &str = "<!-- mneme-end -->";
...
impl MarkerBlock {
    ...
    /// matches what mneme last wrote (safe to overwrite); `Ok(false)`
...
impl MarkerInjector {
    /// Locate the first mneme marker block in `text`. Returns `None` if
    ...
    /// Splice `new_body` into `existing` between mneme sentinels. If no
    ...
    /// Remove the mneme block from `existing`, returning the cleaned
...
/// Parse the start-sentinel header `<!-- mneme-start v1.0 sha256=abc... -->`.
/// Tolerates missing sha (older format).
fn parse_start_header(header: &str) -> (String, Option<String>) {
    // header looks like: "<!-- mneme-start v1.0 sha256=<hex> -->"
    let inner = header
        .trim_start_matches("<!--")
        .trim_end_matches("-->")
        .trim();
    // Split on whitespace: ["mneme-start", "v1.0", "sha256=<hex>"]
    let mut tokens = inner.split_whitespace();
    let _ = tokens.next(); // "mneme-start"
    let version = tokens
        .next()
...
mod tests {
    ...
    fn append_when_no_marker_present() {
        ...
        let injected =
            MarkerInjector::inject(original, "Generated by mneme.", &fake_path(), false).unwrap();
        assert!(injected.starts_with("# My Project"));
        assert!(injected.contains(MARKER_START_PREFIX));
        assert!(injected.contains(MARKER_END));
        assert!(injected.contains("Generated by mneme."));
    }

    #[test]
    fn idempotent_double_injection() {
        let original = "# Doc\n";
        let once = MarkerInjector::inject(original, "BODY-A", &fake_path(), false).unwrap();
        let twice = MarkerInjector::inject(&once, "BODY-A", &fake_path(), false).unwrap();
    ...
    fn refuses_overwrite_when_user_edited_block() {
        let original = "# Doc\n";
        let mut v1 = MarkerInjector::inject(original, "BODY-A", &fake_path(), false).unwrap();
    ...
    fn remove_is_inverse_of_clean_install() {
        let original = "# Doc\n\nNotes\n";
        let injected = MarkerInjector::inject(original, "BODY", &fake_path(), false).unwrap();
    ...
    fn parse_header_extracts_version_and_hash() {
        let (v, h) = parse_start_header("<!-- mneme-start v1.0 sha256=deadbeef -->");
        assert_eq!(v, "1.0");
