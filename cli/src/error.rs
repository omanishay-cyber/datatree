...
pub enum CliError {
    ...
    /// `mneme install` was unable to write the standalone PowerShell
    /// uninstaller (`~/.mneme/uninstall.ps1`) — postmortem §6 Bug H.
    ///
    /// This used to be silently warn-and-continue at the call site
    /// (`warn!(...)` lost in the user's logs). Promoting it to a hard
    /// error surface ensures: (1) `mneme install` exits non-zero when
    /// the recovery path is unavailable, (2) the post-write verification
    /// can return its `io::Error` to the caller intact, and (3)
    /// integration tests can `expect(...)` rather than scrape logs.
    #[error("failed to drop ~/.mneme/uninstall.ps1: {0}")]
    DropUninstaller(#[source] std::io::Error),
...
impl CliError {
    ...
    pub fn exit_code(&self) -> i32 {
        match self {
            ...
            // Bug H: dropping the standalone uninstaller is an IO-class
            // failure from the user's perspective (couldn't write a
            // recovery script). Reuse the IO exit code (2) so existing
            // shells branching on `mneme install` exit codes don't have
            // to learn a new value.
            CliError::DropUninstaller(_) => 2,
            CliError::Other(_) => 1,
        }
    }
}
