...
pub async fn run(args: SnapArgs, _socket_override: Option<PathBuf>) -> CliResult<()> {
    ...
    // HOME-bypass-snap fix: route through PathManager so MNEME_HOME wins.
    let snap_dir = common::paths::PathManager::default_root()
        .root()
        .join("snapshots")
        .join(id.to_string());
...
mod tests {
    ...
    fn utc_stamp_is_well_formed() {
        let s = utc_stamp();
        // Format YYYYMMDD-HHMMSS (15 chars including the dash).
        assert_eq!(s.len(), 15, "stamp len: {s}");
        assert_eq!(&s[8..9], "-", "no dash at index 8: {s}");
        assert!(s[..8].chars().all(|c| c.is_ascii_digit()));
        assert!(s[9..].chars().all(|c| c.is_ascii_digit()));
    }

    #[test]
    fn ymd_epoch_zero_is_1970_01_01() {
        let (y, m, d) = ymd(0);
        assert_eq!((y, m, d), (1970, 1, 1));
    }

    #[tokio::test]
    async fn run_with_missing_graph_db_errors_clearly() {
        ...
        match r {
            Err(CliError::Other(msg)) => {
                assert!(msg.contains("graph.db not found"), "wrong message: {msg}")
            }
            other => panic!("expected Other(graph.db not found), got {other:?}"),
        }
    }
}
