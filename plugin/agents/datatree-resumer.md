...
Step Ledger and compose the `<datatree-resume>` bundle (design §7.3) so
...
2. Fetch the full step ledger:
   - `datatree query --tool step_status --session-id="$SESSION_ID" --json`
3. Fetch the original goal (verbatim from session start):
   - `datatree query --layer history --where "session_id = ? AND tool = '__session_start__' LIMIT 1"`
4. Identify:
...
```json
{
  "additional_context": "<datatree-resume>...</datatree-resume>",
