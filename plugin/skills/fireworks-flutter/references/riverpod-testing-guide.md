...
## The Golden Rule: NEVER Mock Providers

**Use `ProviderContainer` overrides, NOT mocks. Mocking providers hides real behavior.**

When you mock a provider, your test validates the mock's behavior --- not your actual
provider logic. Overrides, on the other hand, replace only the *dependency* while
keeping the real provider code running. This means bugs in your provider's `build()`
or state transitions are caught during testing instead of slipping through.

### WRONG: Mocking the provider itself

```dart
// BAD --- this tests mock behavior, not your real provider
final mockNotifier = MockNotificationNotifier();
when(mockNotifier.state).thenReturn(
  AsyncData(NotificationState.initial()),
);
when(mockNotifier.refreshState()).thenAnswer((_) async {});

// You are testing a mock, not your code!
```

### RIGHT: Overriding dependencies via ProviderContainer

```dart
// GOOD --- real provider runs, only its dependencies are replaced
...
// The REAL notifier runs build(), reads the mock service, and you test actual logic
...
| Aspect | Mocking the provider | Overriding dependencies |
|---|---|---|
| What runs | Mock stub | Real provider code |
| Catches logic bugs | No | Yes |
| Catches state transition bugs | No | Yes |
| Refactor-safe | Fragile | Resilient |

### Scenario 1: Override a service provider with a fake value

```dart
test('loads empty state when service returns nothing', () async {
  final mockService = MockNotificationService();
  when(mockService.fetchNotifications()).thenAnswer((_) async => []);
  when(mockService.getAudioConfiguration()).thenAnswer(
  ...
  final state = await container.read(notificationNotifierProvider.future);
  expect(state.notifications, isEmpty);
});
```

### Scenario 2: Override a repository with an in-memory implementation

```dart
test('fetches products from overridden repository', () async {
  final fakeRepo = InMemoryProductRepository(
    products: [Product(id: '1', name: 'Test Vodka', price: 19.99)],
  );
  ...
  expect(products.first.name, 'Test Vodka');
});
```

### Scenario 3: Override multiple providers for integration-style tests

```dart
test('notification refresh updates state from all dependencies', () async {
  final mockService = MockNotificationService();
  final mockLogger = MockLogger();

  when(mockService.fetchNotifications()).thenAnswer(
    (_) async => [NotificationData(refName: 'Alert', userId: 'u1')],
  );
  when(mockService.getAudioConfiguration()).thenAnswer(
    (_) async => (isAudioMuted: false, isAudioEnabled: true),
  );
