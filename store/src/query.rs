...
impl DbQuery for DefaultQuery {
    ...
    async fn write(&self, w: Write) -> Response<WriteSummary> {
        ...
        if let Err(e) = tx.send(WriteCmd::Single {
            sql: w.sql,
            params: w.params,
            reply: rtx,
        }).await {
            return Response::err(
                DbError::Sqlite(format!("writer channel closed: {}", e)),
                meta(w.layer),
            );
        }
        match rrx.await {
            Ok(Ok(s)) => Response::ok(s, meta(w.layer)),
            Ok(Err(e)) => Response::err(e, meta(w.layer)),
            Err(_) => Response::err(
    ...
    async fn write_batch(&self, ws: Vec<Write>) -> Response<BatchSummary> {
        ...
        for (layer, items) in by_layer {
            ...
            let (rtx, rrx) = oneshot::channel();
            if let Err(e) = tx.send(WriteCmd::Batch { items, reply: rtx }).await {
                return Response::err(
                    DbError::Sqlite(format!("writer channel closed: {}", e)),
                    meta(layer),
                );
