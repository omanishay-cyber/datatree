...
#
#   curl -fsSLo /tmp/install-mac.sh https://github.com/omanishay-cyber/mneme/releases/download/v0.4.0/install-mac.sh
...
MODELS_EOF
    ...
    # skip-if-present guard. The bootstrap stages downloads in MODELS_DIR
    # then `mneme models install --from-path` copies into FINAL_MODELS_DIR.
    # The Bug #232 manifest-merge fix means existing entries here survive
    # even when the staging dir contains only a subset of the assets.
    FINAL_MODELS_DIR="${MNEME_HOME}/models"

    # Portable file-size helper. macOS BSD stat needs `stat -f%z`;
    # try GNU `stat -c%s` first for cross-compat with Linux clones.
    file_size() {
        local _f="$1"
        local _sz
        _sz=$(stat -f%z "${_f}" 2>/dev/null) || _sz=$(stat -c%s "${_f}" 2>/dev/null) || _sz=""
        printf '%s' "${_sz}"
    }
    ...
    for row in ${MODEL_LIST}; do
        ...
        # padding above guarantees fields exist, but bash 3.2 `read` can
        # still leave m_fallback / m_size unset on a literally-empty
        # trailing field. Force-default so downstream doesn't see
        # unbound variables when phi-3's GitHub fallback is empty.
